...
function ScrollVelocityRowImpl({
  ...
  useEffect(() => {
    ...
    const updateSizes = () => {
      ...
      const unitWithGap = bw + GAP_PX
      unitWidth.set(unitWithGap)
      const nextCopies = bw > 0 ? Math.max(3, Math.ceil(cw / unitWithGap) + 2) : 1
  ...
  return (
    ...
    >
      <motion.div
        className="inline-flex transform-gpu items-center will-change-transform select-none gap-x-12"
        style={{ x }}
      >
